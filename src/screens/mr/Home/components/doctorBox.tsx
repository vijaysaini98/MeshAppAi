...
export const DoctorBox = ({
  ...
  const locationDate = (n?: number) => {
    ...
    return (
      <View style={{ flex: 1, flexDirection: "row" }}>
        ...
        {appointmentAdrress && completeAddress?.length >= 50 && (
          <AppText
            weight={BOLD}
            style={{
              textAlign: "left",
  ...
  const successCallbackdr = (item?: any) => {
    let successData = `company_name=${item?.user_name[0]?.Company?.company_name
      ? item?.user_name[0]?.Company?.company_name
      : ""
      }&from=${item?.user_name[0]?.name}&to=${item?.doctor_profile?.name
      }&date=${moment(
        item?.appointment_date ? item?.appointment_date?.created_at : new Date()
      ).format("MMMM Do YYYY")}&payment_method=online&amount=${item?.final_amount ? item?.final_amount : "0"
      }&appointment_id=${item?.id}&token=Bearer `;
    dispatch(downloadPaymentReceipt(successData));
  };

  const successCallback = (data?: any, item?: any) => {
    let successData = `company_name=${data[0]?.Company?.company_name}&from=${data[0]?.name
