...
import { DummyDoctor, locationIcon } from "../../helper/ImageAssets";
...
const DoctorProfile = ({ route }) => {
  ...
  const renderSpecialties = () => {
    const specialties = data?.spec_details || data?.doctor_speciality || [];
    return specialties.map((specialty: any) => (
      <View key={specialty?.id} style={styles.department}>
        <AppText color={WHITE}>
          {specialty?.speciality?.specialization || specialty?.specialization}
        </AppText>
      </View>
    ));
  };
        ...
        <KeyBoardAware style={styles.profileContainer}>
          ...
          >
            <AppText type={THIRTY_EIGHT} weight={MEDIUM}>
              {data?.user_details?.name ?? data?.name}
            </AppText>
            {address?.state != null && address?.state && (
              <View
                style={[
                  styles.homeToolContainer4,
          ...
          <View style={styles.specialtyContainer}>
            ...
            <View style={styles.homeToolContainer2}>{renderSpecialties()}</View>
          </View>
        </KeyBoardAware>
        <Button
