...
const PaymentReceipt = ({ route }) => {
  ...
  const paymentReceipt = [
    {
      id: "1",
      left: "Company Name :-",
      right: paymentReceiptData[0]?.Company?.company_name,
    },
    {
      id: "2",
      left: "From :-",
      right: paymentReceiptData[0]?.name,
    },
    {
      id: "3",
      left: "To :-",
  ...
  const onDownload = () => {
    let data = `company_name=${paymentReceiptData[0]?.Company?.company_name
      }&from=${paymentReceiptData[0]?.name}&to=${details?.doctor_profile?.name
      }&date=${moment(
        details?.appointment_date
          ? details?.appointment_date?.created_at
          : new Date()
